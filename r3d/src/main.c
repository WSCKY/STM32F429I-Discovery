...
static const r3d_drawcall_t meshes[] = {
	{ R3D_PRIMITIVE_TYPE_TRIANGLES, (void *)vertices0,
	  sizeof(vertex_t), sizeof(vertices0) / sizeof(vertex_t), 0 },
	{ R3D_PRIMITIVE_TYPE_TRIANGLES, (void *)vertices1,
	  sizeof(vertex_t), sizeof(vertices1) / sizeof(vertex_t), 0 },
	{ R3D_PRIMITIVE_TYPE_TRIANGLES, (void *)vertices2,
...
static vec4_t fragment_shader(const vs_to_fs_t *in)
{
	...
	return vec4(ambient + diffuse * c.vect.color.r + specular,
	            ambient + diffuse * c.vect.color.g + specular,
	            ambient + diffuse * c.vect.color.b + specular, 1.0f);
}
...
static void init(void)
{
	...
	itoa(meshes[mesh].count / 3, (char *)info_str, 10);
	strcat((char *)info_str, " tris");
...
static void render(void)
{
	...
	r3d_shader = shader;
	r3d_draw((r3d_drawcall_t *)&meshes[mesh]);
...
{
	if (--fps_countdown == 0) {
		itoa(fps, (char *)fps_str, 10);
		strcat((char *)fps_str, " FPS");
		fps = 0;
		fps_countdown = 100;
	}
	time += 0.01f;
