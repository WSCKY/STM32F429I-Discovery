...
typedef void (*r3d_vertexshader_func)(void *in, float *out);
typedef vec4_t (*r3d_fragmentshader_func)(float *in);
typedef struct {
...
{
	int ui = (int)uv.vect.raw.x;
	int vi = (int)uv.vect.raw.y;
	if (uv.vect.raw.x < 0) ui--;
	if (uv.vect.raw.y < 0) vi--;
	uv.vect.raw.x -= ui;
	uv.vect.raw.y -= vi;
	uv.vect.raw.x *= texture->width - 1;
	uv.vect.raw.y *= texture->height - 1;
	ui = uv.vect.raw.x;
	vi = uv.vect.raw.y;
